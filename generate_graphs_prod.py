...
from openpyxl import load_workbook  # Ensure this import is at the top
...
# Prepare data for Excel and PDF (only include prod releases)
excel_data = []
for team, data in team_release_data.items():
    # Only include teams that had prod releases
    if data['prod'] > 0:
        ...
        excel_data.append([
            ...
            data['fast'],
            data['slow'],
            ', '.join(data['branches']),  # Joined branch names with 'prod'
            releases_with_line_breaks
        ])
...
with PdfPages(pdf_file_path) as pdf:
    ...
    # Bar chart: Number of prod releases by team
    teams = [team for team in team_release_data.keys() if team_release_data[team]['prod'] > 0]
    pr_counts = [team_release_data[team]['prod'] for team in teams]
    bars = ax1.bar(teams, pr_counts, color='blue')

    ax1.set_xticks(range(len(teams)))
    ax1.set_xticklabels(teams, rotation=45, ha='right')
    ax1.set_xlabel('Team')
    ax1.set_ylabel('Number of Prod Releases')
    ax1.set_title('Number of Prod Releases by Team')
    ...
    if total_fast_releases > 0 or total_slow_releases > 0:
        ax2.pie(release_counts, 
                ...
                startangle=90, 
                colors=['green', 'brown'])
        ax2.set_title('Fast vs Slow Prod Releases')
    else:
    ...
    table_data = [[team, data['total'], data['fast'], data['slow'], ', '.join(data['branches'])] 
                  for team, data in team_release_data.items() if data['prod'] > 0]
